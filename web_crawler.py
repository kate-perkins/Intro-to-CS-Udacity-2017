# 
# Web crawler with depth constraint
#
#######

# Modify the crawl_web procedure [I wrote this earlier] to take a second parameter,
...
# get_page test method given by Udacity #
...
def get_next_target(page):
    ...
    if start_link == -1:
        return None, 0
    end_link = page.find('"', start_link + len('<a href=') + 1)
    url = page[start_link + len('<a href=') + 1:end_link]
    return url, end_link
...
def crawl_web(seed,max_depth):
    ...
    crawleddepth = []
    while tocrawl:
        if tocrawl[-1][1] <= max_depth:
            depth = tocrawl[-1].pop() + 1
            page = tocrawl.pop()[0]
            if page not in crawled or crawleddepth[crawled.index(page)] > depth: #previous recorded depth > current depth
                new_links = get_all_links(get_page(page))
                for a in new_links:
                    tocrawl.append([a, depth])
                if page not in crawled:
                    crawled.append(page)
                    crawleddepth.append(depth)
        else:
            tocrawl.pop()
    return crawled
